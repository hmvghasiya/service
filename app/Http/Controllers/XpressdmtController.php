...
class XpressdmtController extends Controller
{
    protected $api;
    ...
    public function payment(Request $post)
    {
        ...
        switch ($post->type) {
            ...
            case 'transfer':
                return $this->payouttransfer($post, $userdata);
                break;
    ...
    public function payouttransfer($post)
    {
        $outputs['statuscode'] = "TXN";
        $user = User::where('id', $post->user_id)->first();
        $api  = Api::where('code', 'xpressdmt')->first();
        $url  = $api->url;
        ...
        if($api->status == '0'){
            $outputs['data'][] = array(
                ...
                'data'   => [
                    "statuscode" => "ERR",
                    "status"     => "Service Down For Sometime",
                    "message"    => "Service Down For Sometime",
                ]
            );
        }
        ...
        $parameter = [
            ...
            "token"    => $api->username,
            'ip'       => $post->ip(),
            'callback' => url('api/callback/update/xpress')
        ];
        ...
        if($user->mainwallet < $post->amount + $post->charge){
            ...
            $charge = 0;
                ...
                if($post->amount >= 10 && $post->amount <= 5000){
                    $myprovider = Provider::where('recharge1', 'dmt6')->first();
                }elseif($amount>5000 && $amount<=10000){
                    $myprovider = Provider::where('recharge1', 'dmt7')->first();
                }elseif($amount>10000 && $amount<=25000){
                    $myprovider = Provider::where('recharge1', 'dmt8')->first();
                }elseif($amount>25000 && $amount<=50000){
                    $myprovider = Provider::where('recharge1', 'dmt9')->first();
                }else{
                    $myprovider = Provider::where('recharge1', 'dmt9')->first();
                }

                $charge += \Myhelper::getCommission($amount, $user->scheme_id, $myprovider->id, $user->role->slug);
            

            $bank = Mahabank::where('bankid', $post->benebank)->first();
          //  $myprovider = Provider::where('recharge1', 'payourtransfer')->first();
            $insert = [
                'api_id' => $api->id,
                ...
                'option5'  => $post->aadhar."/".$post->pancard,
                'status'  => 'success',
                'user_id' => $user->id,
                'credit_by' => $user->id,
                'product' => 'dmt',
            ...
            $previousrecharge = Report::where('number', $post->beneaccount)->where('amount', $post->amount)->where('provider_id', $post->provider_id)->whereBetween('created_at', [Carbon::now()->subSeconds(2)->format('Y-m-d H:i:s'), Carbon::now()->format('Y-m-d H:i:s')])->count();
            ...
            $transaction = User::where('id', $user->id)->decrement('mainwallet', $post->amount + $charge);
            if(!$transaction){
                $outputs['data'][] = array(
                    'amount' => $post->amount,
                    'status' => 'TXF',
                ...
                try {
                   ...
                   // dd([$url,$header,$parameter,json_encode($parameter),$header,$result]);
                    if($result['response'] == ''){
                        $outputs['data'][] = array(
                            'amount' => $post->amount,
                            'status' => 'TXN',
                        ...
                        \Myhelper::commission(Report::find($report->id));
                    }

                    $response = json_decode($result['response']);
                    if(isset($response->status) && in_array($response->status, ['TXN', 'TUP'])){
                        Report::where('id', $report->id)->update(['refno' => $response->rrn]);
                        \Myhelper::commission(Report::find($report->id));
                        $outputs['data'][] = array(
                            'amount' => $post->amount,
                            'status' => 'TXN',
                    ...
                    }elseif(isset($response->status) && in_array($response->status, ['TXF', 'ERR', 'BPR'])){
                        User::where('id', $user->id)->increment('mainwallet', $post->amount + $charge);
                        Report::where('id', $report->id)->update(['status' => "failed", "refno" => isset($response->rrn) ? $response->rrn : $response->message]);
                        $outputs['data'][] = array(
                            'amount' => $post->amount,
                            'status' => 'TXF',
                            'data' => [
