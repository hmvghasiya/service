...
class GstRegController extends Controller
{
    ...
    public function user_index()
    {
        ...
        if (\Myhelper::hasRole('retailer')) {
            return redirect()->route('gst_reg.create');
        }else{
            abort(403);
        }
        
        // return view('gst_reg.user_index');
    }

    public function view(Request $request,$id)
    {
        $gst_reg=GstReg::find($id);
        return view('gst_reg.view',compact('gst_reg'));
    }
    ...
    public function create()
    {
       ...
       return view('gst_reg.addedit',compact('state'));
    }

    public function loan_data(Request $request)
    {
        $loan_id=$request->loan_id;
        return view('gst_reg.partial.loan_data',compact('loan_id'));
    }
        ...
        return Datatables::of($sql)
              ->addColumn('action',function($data){
                ...
                $string="";
                  ...
                  return view('gst_reg.partial.action',compact('data'));
              })
              ->editColumn('f_name',function($data){

                return  view('gst_reg.partial.name',compact('data'));
              })
              ->editColumn('loan_detail',function($data){

                return  view('gst_reg.partial.loan_detail',compact('data'));
              })
              ->editColumn('address',function($data){

                return  view('gst_reg.partial.address',compact('data'));
              })
    ...
    {
         $sql=GstReg::select("*")->where('user_id',\Auth::user()->id);
        return Datatables::of($sql)
              ->addColumn('action',function($data){
                ...
                $string="";
                  ...
                  return view('gst_reg.partial.user_action',compact('data'));
              })
              ->editColumn('f_name',function($data){
    ...
    public function store(GstRegRequest $request)
    {
        ...
        if (isset($request->photo) && $request->photo != null) {
            ...
            $imageName = UPLOAD_FILE($request,'photo',ADMIN_GST_REG_PHOTO_UPLOAD_PATH());
            if ($imageName !="") {
              $res->photo = $imageName;
            }
        } if (isset($request->pancard_photo) && $request->pancard_photo != null) {

            $imageName = UPLOAD_FILE($request,'pancard_photo',ADMIN_GST_REG_PAN_UPLOAD_PATH());
            if ($imageName !="") {
              $res->pancard_photo = $imageName;
            }
        } if (isset($request->bank_statement_photo) && $request->bank_statement_photo != null) {

            $imageName = UPLOAD_FILE($request,'bank_statement_photo',ADMIN_GST_REG_BANK_STATE_UPLOAD_PATH());
            if ($imageName !="") {
        ...
        if (isset($request->addhar_card_photo) && $request->addhar_card_photo != null) {
            ...
            $imageName = UPLOAD_FILE($request,'addhar_card_photo',ADMIN_GST_REG_ADHAR_UPLOAD_PATH());
            if ($imageName !="") {
        ...
        if (isset($request->address_proof) && $request->address_proof != null) {
            ...
            $imageName = UPLOAD_FILE($request,'address_proof',ADMIN_GST_REG_ADDRESS_UPLOAD_PATH());
            if ($imageName !="") {
        ...
        if (isset($request->signature) && $request->signature != null ) {
            ...
            $imageName = UPLOAD_FILE($request,'signature',ADMIN_GST_REG_SIGN_UPLOAD_PATH());
            if ($imageName !="") {
         ...
         if (isset($request->itr_3_year) && $request->itr_3_year != null) {
            ...
            $imageName = UPLOAD_FILE($request,'itr_3_year',ADMIN_GST_REG_ITR_UPLOAD_PATH());
            if ($imageName !="") {
        ...
        $msg="Your Gst Registration Form Submited Successfully";
        $url=route('gst_reg.index');
