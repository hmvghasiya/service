...
use App\Model\RationCard;
use App\Model\Utiid;
use App\Model\Permission;
use App\User;
use App\Model\Commission;
use App\Http\Requests\RationCardRequest;
...
class RationCardController extends Controller
{
    ...
    public function user_index()
    {
        if (\Myhelper::hasRole('retailer')) {
            ...
            return redirect()->route('ration_card.create');
        }else{
            abort(403);
    ...
    {
        $ration_card=RationCard::find($id);
        return view('ration_card.view',compact('ration_card'));
    }
    ...
    public function create()
    {
       ...
       return view('ration_card.addedit',compact('state'));
    }

    public function loan_data(Request $request)
    {
        $loan_id=$request->loan_id;
        return view('ration_card.partial.loan_data',compact('loan_id'));
    }

    public function any_data(Request $request)
    {
         $sql=RationCard::select("*");
        return Datatables::of($sql)
              ->addColumn('action',function($data){
                ...
                $string="";
                  ...
                  return view('ration_card.partial.action',compact('data'));
              })
              ->editColumn('f_name',function($data){

                return  view('ration_card.partial.name',compact('data'));
              })
              ->editColumn('loan_detail',function($data){

                return  view('ration_card.partial.loan_detail',compact('data'));
              })
              ->editColumn('address',function($data){

                return  view('ration_card.partial.address',compact('data'));
              })
    ...
    {
         $sql=RationCard::select("*")->where('user_id',\Auth::user()->id);
        return Datatables::of($sql)
              ->addColumn('action',function($data){
                ...
                $string="";
                  ...
                  return view('ration_card.partial.user_action',compact('data'));
              })
              ->editColumn('f_name',function($data){
    ...
    public function single_status_change(Request $request)
    {
        ...
        $res=RationCard::find($request->id);
        if ($status==1) {
            $res->status=1;
        }elseif ($status==2) {
            $res->status=2;
        ...
        $res->save();
        return view('ration_card.partial.reason_modal',compact('res'));
    }

    public function reason(Request $request)
    {
        $id=$request->id;
        $res=RationCard::find($id);
    ...
    public function store(RationCardRequest $request)
    {
        ...
        if (isset($request->photo) && $request->photo != null) {
            ...
            $imageName = UPLOAD_FILE($request,'photo',ADMIN_RATION_CARD_PHOTO_UPLOAD_PATH());
            if ($imageName !="") {
              $res->photo = $imageName;
            }
        } if (isset($request->pancard_photo) && $request->pancard_photo != null) {

            $imageName = UPLOAD_FILE($request,'pancard_photo',ADMIN_RATION_CARD_PAN_UPLOAD_PATH());
            if ($imageName !="") {
              $res->pancard_photo = $imageName;
            }
        } if (isset($request->bank_statement_photo) && $request->bank_statement_photo != null) {

            $imageName = UPLOAD_FILE($request,'bank_statement_photo',ADMIN_RATION_CARD_BANK_STATE_UPLOAD_PATH());
            if ($imageName !="") {
        ...
        if (isset($request->addhar_card_photo) && $request->addhar_card_photo != null) {
            ...
            $imageName = UPLOAD_FILE($request,'addhar_card_photo',ADMIN_RATION_CARD_ADHAR_UPLOAD_PATH());
            if ($imageName !="") {
        ...
        if (isset($request->address_proof) && $request->address_proof != null) {
            ...
            $imageName = UPLOAD_FILE($request,'address_proof',ADMIN_RATION_CARD_ADDRESS_UPLOAD_PATH());
            if ($imageName !="") {
        ...
        if (isset($request->nationalilty) && $request->nationalilty != null ) {
            ...
            $imageName = UPLOAD_FILE($request,'nationalilty',ADMIN_RATION_CARD_NATION_UPLOAD_PATH());
            if ($imageName !="") {
         ...
         if (isset($request->itr_3_year) && $request->itr_3_year != null) {
            ...
            $imageName = UPLOAD_FILE($request,'itr_3_year',ADMIN_RATION_CARD_ITR_UPLOAD_PATH());
            if ($imageName !="") {
        ...
        $msg="Your Ration Card Form Submited Successfully";
        $url=route('ration_card.index');
