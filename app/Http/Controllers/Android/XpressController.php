...
class XpressController extends Controller
{
    protected $api;
        ...
        switch ($post->type) {
            ...
            case 'transfer':
                ...
                // }
                  ...
                  return $this->payouttransfer($post);
                break;
     ...
     public function payouttransfer($post)
    {
        ...
        $outputs['message'] = "Transaction Successfull";
        ...
        $parameter = [
            ...
            "token"    => $api->username,
            'ip'       => $post->ip(),
            'callback' => url('api/callback/update/payoutdmt')
        ];
                ...
                try {
                    ...
                    if(isset($response->status) && in_array($response->status, ['TXN', 'TUP'])){
                        ...
                        $outputs['data'] = array([
                            'amount' => $amount, 
                            'status' => 'success', 
                            'message'=> 'Transaction Successfull2', 
                        ...
                        $outputs['data'][] = array([
                            'amount' => $amount, 
                            'status' => 'success', 
                            'message'=> 'Transaction Successfull1', 
                            "rrn"    => isset($response->rrn) ? $response->rrn : $response->message
                        ]);
                    ...
                    if(isset($report)){
                        \Myhelper::commission(Report::find($report->id));
                        $output = ['amount' => $amount, 'status'=> 'pending', 'message'=> 'pending', "rrn" => (isset($response->Data[0]->rrn))?$response->Data[0]->rrn : "Transaction Under Process"];
                        // $outputs['data'][] = array([
                        //     'amount' => $amount, 
                        //     'status' => 'pending', 
