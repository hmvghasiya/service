...
use App\Model\PrepaidcardKyc;
use App\Model\Utiid;
use App\Model\Permission;
use App\User;
use App\Model\Commission;
use App\Http\Requests\PrepaidcardKycRequest;
use Yajra\Datatables\Datatables;
use Crypt;


class PrepaidcardKycController extends Controller
{
    ...
    public function index()
    {
        if (\Myhelper::hasNotRole('retailer')) {
            return view('prepaidcard_kyc.index');
        }else{
            abort(403);
        }
    }
    ...
    public function user_index()
    {
        if (\Myhelper::hasRole('retailer')) {
            return redirect()->route('prepaidcard_kyc.create');
        }else{
            abort(403);
        }
        
        // return view('prepaidcard_kyc.user_index');
    }

    public function view(Request $request,$id)
    {
        $prepaidcard_kyc=PrepaidcardKyc::find($id);
        return view('prepaidcard_kyc.view',compact('prepaidcard_kyc'));
    }
    ...
    public function create()
    {
       ...
       return view('prepaidcard_kyc.addedit',compact('state'));
    }

    public function loan_data(Request $request)
    {
        $loan_id=$request->loan_id;
        return view('prepaidcard_kyc.partial.loan_data',compact('loan_id'));
    }

    public function any_data(Request $request)
    {
         $sql=PrepaidcardKyc::select("*");
        return Datatables::of($sql)
              ->addColumn('action',function($data){
                ...
                $string="";
                  ...
                  return view('prepaidcard_kyc.partial.action',compact('data'));
              })
              ->editColumn('name',function($data){

                return  view('prepaidcard_kyc.partial.name',compact('data'));
              })
              ->editColumn('loan_detail',function($data){

                return  view('prepaidcard_kyc.partial.loan_detail',compact('data'));
              })
              ->editColumn('address',function($data){

                return  view('prepaidcard_kyc.partial.address',compact('data'));
              })
    ...
    {
         $sql=PrepaidcardKyc::select("*")->where('user_id',\Auth::user()->id);
        return Datatables::of($sql)
              ->addColumn('action',function($data){
                ...
                $string="";
                  ...
                  return view('prepaidcard_kyc.partial.user_action',compact('data'));
              })
              ->editColumn('name',function($data){
    ...
    public function single_status_change(Request $request)
    {
        ...
        $res=PrepaidcardKyc::find($request->id);
        if ($status==1) {
            $res->status=1;
        }elseif ($status==2) {
            $res->status=2;
        ...
        $res->save();
        return view('prepaidcard_kyc.partial.reason_modal',compact('res'));
    }

    public function reason(Request $request)
    {
        $id=$request->id;
        $res=PrepaidcardKyc::find($id);
    ...
    public function store(PrepaidcardKycRequest $request)
    {
        ...
        $res=new PrepaidcardKyc;

        if (isset($request->photo) && $request->photo != null) {

            $imageName = UPLOAD_FILE($request,'photo',ADMIN_PREPAIDCARD_KYC_PHOTO_UPLOAD_PATH());
            if ($imageName !="") {
              $res->photo = $imageName;
            }
        } if (isset($request->pancard_photo) && $request->pancard_photo != null) {

            $imageName = UPLOAD_FILE($request,'pancard_photo',ADMIN_PREPAIDCARD_KYC_PAN_UPLOAD_PATH());
            if ($imageName !="") {
              $res->pancard_photo = $imageName;
            }
        } if (isset($request->passbook_photo) && $request->passbook_photo != null) {

            $imageName = UPLOAD_FILE($request,'passbook_photo',ADMIN_PREPAIDCARD_KYC_PASS_BOOK_UPLOAD_PATH());
            if ($imageName !="") {
              $res->passbook_photo = $imageName;
            }
        } 
        if (isset($request->addhar_card_photo) && $request->addhar_card_photo != null) {

            $imageName = UPLOAD_FILE($request,'addhar_card_photo',ADMIN_PREPAIDCARD_KYC_ADHAR_UPLOAD_PATH());
            if ($imageName !="") {
        ...
        $res->father_name=$request->father_name;
        ...
        $res->full_address=$request->full_address;
        ...
        $msg="Your Prepaidcard kyc Submited Successfully";
        $url=route('prepaidcard_kyc.index');
