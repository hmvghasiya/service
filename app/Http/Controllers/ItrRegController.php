...
use App\Model\ItrReg;
use App\Model\Utiid;
use App\Model\Permission;
use App\User;
use App\Model\Commission;
use App\Http\Requests\ItrRegRequest;
use Yajra\Datatables\Datatables;
use Crypt;


class ItrRegController extends Controller
{
    ...
    public function user_index()
    {
        if (\Myhelper::hasRole('retailer')) {
            return redirect()->route('itr_reg.create');
        }else{
            abort(403);
        }

        // return view('itr_reg.user_index');
    }

    public function view(Request $request,$id)
    {
        $itr_reg=ItrReg::find($id);
        return view('itr_reg.view',compact('itr_reg'));
    }
    ...
    public function create()
    {
       ...
       return view('itr_reg.addedit',compact('state'));
    }

    public function loan_data(Request $request)
    {
        $loan_id=$request->loan_id;
        return view('itr_reg.partial.loan_data',compact('loan_id'));
    }
        ...
        return Datatables::of($sql)
              ->addColumn('action',function($data){
                ...
                $string="";
                  ...
                  return view('itr_reg.partial.action',compact('data'));
              })
              ->editColumn('f_name',function($data){

                return  view('itr_reg.partial.name',compact('data'));
              })
              ->editColumn('loan_detail',function($data){

                return  view('itr_reg.partial.loan_detail',compact('data'));
              })
              ->editColumn('address',function($data){

                return  view('itr_reg.partial.address',compact('data'));
              })
    ...
    {
         $sql=ItrReg::select("*")->where('user_id',\Auth::user()->id);
        return Datatables::of($sql)
              ->addColumn('action',function($data){
                ...
                $string="";
                  ...
                  return view('itr_reg.partial.user_action',compact('data'));
              })
              ->editColumn('f_name',function($data){
    ...
    public function single_status_change(Request $request)
    {
        ...
        $res->save();
        return view('itr_reg.partial.reason_modal',compact('res'));
    }
    ...
    public function store(ItrRegRequest $request)
    {
        ...
        if (isset($request->photo) && $request->photo != null) {
            ...
            $imageName = UPLOAD_FILE($request,'photo',ADMIN_ITR_REG_PHOTO_UPLOAD_PATH());
            if ($imageName !="") {
              $res->photo = $imageName;
            }
        } if (isset($request->pancard_photo) && $request->pancard_photo != null) {

            $imageName = UPLOAD_FILE($request,'pancard_photo',ADMIN_ITR_REG_PAN_UPLOAD_PATH());
            if ($imageName !="") {
              $res->pancard_photo = $imageName;
            }
        } if (isset($request->bank_statement_photo) && $request->bank_statement_photo != null) {

            $imageName = UPLOAD_FILE($request,'bank_statement_photo',ADMIN_ITR_REG_BANK_STATE_UPLOAD_PATH());
            if ($imageName !="") {
        ...
        if (isset($request->addhar_card_photo) && $request->addhar_card_photo != null) {
            ...
            $imageName = UPLOAD_FILE($request,'addhar_card_photo',ADMIN_ITR_REG_ADHAR_UPLOAD_PATH());
            if ($imageName !="") {
        ...
        if (isset($request->address_proof) && $request->address_proof != null) {
            ...
            $imageName = UPLOAD_FILE($request,'address_proof',ADMIN_ITR_REG_ADDRESS_UPLOAD_PATH());
            if ($imageName !="") {
        ...
        if (isset($request->signature) && $request->signature != null ) {
            ...
            $imageName = UPLOAD_FILE($request,'signature',ADMIN_ITR_REG_SIGN_UPLOAD_PATH());
            if ($imageName !="") {
         ...
         if (isset($request->itr_3_year) && $request->itr_3_year != null) {
            ...
            $imageName = UPLOAD_FILE($request,'itr_3_year',ADMIN_ITR_REG_ITR_UPLOAD_PATH());
            if ($imageName !="") {
        ...
        $msg="Your Itr Registration Form Submited Successfully";
        $url=route('itr_reg.index');
