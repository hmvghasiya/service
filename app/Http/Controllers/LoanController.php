...
class LoanController extends Controller
{
    ...
    public function user_index()
    {
        ...
        // return view('loan.user_index');
    }

    public function view(Request $request,$id)
    {
        $loan=Loan::find($id);
        return view('loan.view',compact('loan'));
    }
    ...
    public function create()
    {
       ...
       return view('loan.addedit',compact('state'));
    }

    public function loan_data(Request $request)
    {
        $loan_id=$request->loan_id;
        return view('loan.partial.loan_data',compact('loan_id'));
    }
        ...
        return Datatables::of($sql)
              ->addColumn('action',function($data){
                ...
                $string="";
                  ...
                  return view('loan.partial.action',compact('data'));
              })
              ->editColumn('f_name',function($data){

                return  view('loan.partial.name',compact('data'));
              })
              ->editColumn('loan_detail',function($data){

                return  view('loan.partial.loan_detail',compact('data'));
              })
              ->editColumn('address',function($data){

                return  view('loan.partial.address',compact('data'));
              })
    ...
    {
         $sql=Loan::select("*")->where('user_id',\Auth::user()->id);
        return Datatables::of($sql)
              ->addColumn('action',function($data){
                ...
                $string="";
                  ...
                  return view('loan.partial.user_action',compact('data'));
              })
              ->editColumn('f_name',function($data){
    ...
    public function single_status_change(Request $request)
    {
        ...
        $res->save();
        return view('loan.partial.reason_modal',compact('res'));
    }
    ...
    public function store(LoanRequest $request)
    {
        ...
        if (isset($request->photo) && $request->photo != null) {
            ...
            $imageName = UPLOAD_FILE($request,'photo',ADMIN_LOAN_PHOTO_UPLOAD_PATH());
            if ($imageName !="") {
              $res->photo = $imageName;
            }
        } if (isset($request->pancard_photo) && $request->pancard_photo != null) {

            $imageName = UPLOAD_FILE($request,'pancard_photo',ADMIN_LOAN_PAN_UPLOAD_PATH());
            if ($imageName !="") {
              $res->pancard_photo = $imageName;
            }
        } if (isset($request->bank_statement_photo) && $request->bank_statement_photo != null) {

            $imageName = UPLOAD_FILE($request,'bank_statement_photo',ADMIN_LOAN_BANK_STATE_UPLOAD_PATH());
            if ($imageName !="") {
        ...
        if (isset($request->addhar_card_photo) && $request->addhar_card_photo != null) {
            ...
            $imageName = UPLOAD_FILE($request,'addhar_card_photo',ADMIN_LOAN_ADHAR_UPLOAD_PATH());
            if ($imageName !="") {
        ...
        if (isset($request->address_proof) && $request->address_proof != null) {
            ...
            $imageName = UPLOAD_FILE($request,'address_proof',ADMIN_LOAN_ADDRESS_UPLOAD_PATH());
            if ($imageName !="") {
              $res->address_proof = $imageName;
            }
        } 
        if (isset($request->nationalilty) && $request->nationalilty != null && $request->loan_type==1) {

            $imageName = UPLOAD_FILE($request,'nationalilty',ADMIN_LOAN_NATION_UPLOAD_PATH());
            if ($imageName !="") {
         ...
         if (isset($request->itr_3_year) && $request->itr_3_year != null) {
            ...
            $imageName = UPLOAD_FILE($request,'itr_3_year',ADMIN_LOAN_ITR_UPLOAD_PATH());
            if ($imageName !="") {
        ...
        $msg="Your Loan Registration Form Submited Successfully";
