    ...
    <div class="row">
        <div class="col-sm-4">
            <div class="panel panel-default">
                ...
                <form id="serachForm" action="{{route('edmtpay')}}" method="post">
                    {{ csrf_field() }}
...
<div id="beneficiaryModal" class="modal fade" role="dialog" data-backdrop="false" data-keyboard="false">
    ...
    <!-- Modal content-->
        <div class="modal-content">
            ...
            <form action="{{route('edmtpay')}}" method="post" id="beneficiaryForm">
...
<div id="otpModal" class="modal fade" role="dialog" data-backdrop="false" data-keyboard="false">
    <div class="modal-dialog modal-sm">
        ...
        <div class="modal-content">
            ...
            <form action="{{route('edmtpay')}}" method="post" id="otpForm">
                {{ csrf_field() }}
                <div class="modal-body">
...
<div id="transferModal" class="modal fade" role="dialog" data-backdrop="false" data-keyboard="false">
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            ...
            <form action="{{route('edmtpay')}}" method="post" id="transferForm">
                {{ csrf_field() }}
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" placeholder="Enter value" name="aadhar" step="any" required>
                            </div>
                        </div>
                    </div>
                    <div class="row">
...
<div id="registrationModal" class="modal fade" role="dialog" data-backdrop="false" data-keyboard="false">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{route('edmtpay')}}" method="post" id="registrationForm">
                {{ csrf_field() }}
                <div class="modal-body">
                    ...
                    <div  class="row">
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input type="text" class="form-control mydate" name="dob" required="" placeholder="Enter name">
                        </div>
                    </div>
...
<script type="text/javascript">
                    ...
                    success:function(data){
                        ...
                        if(data.statuscode == "TXN"){
                            ...
                            if(type == "registration" || type == "verification"){
                                ...
                                $('#otpModal').find('[name="benemobile"]').val(data.message.data.otp_ref_id);
                            }
                            var mobile = form.find('[name="mobile"]').val();
                ...
                success:function(data){
                    ...
                    $.each(data.data , function(index, val){
                        if(val.data.statuscode == "TXN" || val.data.statuscode == "TUP"){
                            ...
                            out += '<li class="list-group-item alert alert-danger no-margin mb-10"><strong>Rs.  '+val.amount+'</strong><span class="pull-right">'+val.data.message+'</span></li>';
                        }
                    });
    ...
    function setVerifyData(data) {
        $('.name').text(data.message.data.name);
        $('.mobile').text(data.message.data.mobile);
        $('.totallimit').text( parseInt(data.message.data.total_limit));
        $('.usedlimit').text( parseInt(data.message.data.used_limit) );
        $('.remainlimit').text( parseInt(data.message.data.available_limit) );
        $('[name="mobile"]').val(data.message.data.mobile);
        $('[name="name"]').val(data.message.data.name);
        $('#rname').val(data.message.data.name);
        $('#rlimit').val(parseInt(data.message.data.available_limit));
        ...
        if(data.benedata.length > 0){
            ...
            $.each(data.benedata , function(index, beneficiary) {
                out += `<tr>
                        ...
                        <td>`+beneficiary.account+` <br> (`+beneficiary.ifsc+`)<br> ( `+beneficiary.bank+` )</td>
                        <td>`;
                out +=`<button class="btn bg-slate btn-xs legitRipple" onclick="sendMoney('`+data.message.data.mobile+`','`+data.message.data.name+`','`+beneficiary.recipient_id+`', '`+beneficiary.account+`', '`+beneficiary.ifsc+`', '`+beneficiary.recipient_name+`', '`+beneficiary.recipient_mobile+`', '`+beneficiary.bank+`')"><i class="fa fa-paper-plane"></i> Send</button>`;
                out +=`</td>
                    </tr>`;
            });
    ...
    function sendMoney(mobile, name, rid, beneaccount, beneifsc, benename, benemobile, bankname) {
