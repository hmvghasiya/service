@extends('layouts.app')
@section('title', 'Digital Signature Registration')
@section('pagetitle', 'Digital Signature Registration')
@section('content')
<div class="content">
    <form class="FromSubmit" action="{{ route('digital_signature.store') }}" id="rationform" method="post" enctype="multipart/form-data">
        {{ csrf_field() }}
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="panel-heading">
                    ...
                    {{-- @if ($role || sizeOf($roles) > 0) --}}
                        <div class="heading-elements">
                            <a href="{{route('digital_signature.create')}}"><button type="button" class="btn btn-sm bg-slate btn-raised heading-btn legitRipple">
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        $(document).on('change', '.loan_type', function(event) {
            ...
            // alert(loan_id);
            $.ajax({
                url: '{{route('digital_signature.loan_data')}}',
                type: 'GET',
...
<script type="text/javascript">
    ...
    var oTable = $('#datatable').DataTable({
      ...
      responsive: true,
      ajax: {
            url:'{!! route('digital_signature.user_any_data') !!}',
            data: function (d) {
    ...
    $(document).on("click",".change_rat_status",function(){
      ...
      .then((willDelete) => {
        if (willDelete) {
              ...
              $.ajax({
                type:"POST",
                url:"{{route('digital_signature.single_status_change')}}",
