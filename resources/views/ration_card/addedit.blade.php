@extends('layouts.app')
@section('title', 'Ration Card')
@section('pagetitle', 'Ration Card')
@section('content')
<div class="content">
    <form class="FromSubmit" action="{{ route('ration_card.store') }}" id="rationform" method="post" enctype="multipart/form-data">
        {{ csrf_field() }}
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="panel-heading">
                    ...
                    {{-- @if ($role || sizeOf($roles) > 0) --}}
                        <div class="heading-elements">
                            <a href="{{route('ration_card.create')}}"><button type="button" class="btn btn-sm bg-slate btn-raised heading-btn legitRipple">
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        $(document).on('change', '.loan_type', function(event) {
            ...
            // alert(loan_id);
            $.ajax({
                url: '{{route('ration_card.loan_data')}}',
                type: 'GET',
...
<script type="text/javascript">
    ...
    var oTable = $('#datatable').DataTable({
      ...
      responsive: true,
      ajax: {
            url:'{!! route('ration_card.user_any_data') !!}',
            data: function (d) {
    ...
    $(document).on("click",".change_rat_status",function(){
      ...
      .then((willDelete) => {
        if (willDelete) {
              ...
              $.ajax({
                type:"POST",
                url:"{{route('ration_card.single_status_change')}}",
